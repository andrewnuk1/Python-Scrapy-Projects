...
__contributor__ = 'Andrew Neville'
...
class TwitterSearch:
    ...
    def search(self, query):
        ...
        while response is not None and continue_search and response['items_html'] is not None:
            ...
            if len(tweets) == 0:
                print "ending now due to no tweets"
                break
...
class TwitterSearchImpl(TwitterSearch):
    ...
    def save_tweets(self, tweets):
        """
        Print out Tweets and user_name to screen and xls file.
        :return:
        """
        for tweet in tweets:
            ...
            if tweet['created_at'] is not None:
                ...
                fmt = "%Y-%m-%d %H:%M:%S"
                print "%i [%s] - %s" % (self.counter, t.strftime(fmt),tweet['user_name'])
                print "%i [%s] - %s" % (self.counter, t.strftime(fmt),tweet['text'])

                # write to excel but in rows of max 50,000 then move to new columns
                sheet1.write((self.counter-1)-(int((self.counter-1)/50000)*50000),0 + (int((self.counter-1)/50000)*2), "%i [%s] - %s" % (self.counter, t.strftime(fmt),tweet['user_name']))
                sheet1.write((self.counter-1)-(int((self.counter-1)/50000)*50000),1 + (int((self.counter-1)/50000)*2), "%i [%s] - %s" % (self.counter, t.strftime(fmt),tweet['text']))
...
if __name__ == '__main__':
    twit = TwitterSearchImpl(0, 5, 100000)
    twit.search("@Deliveroo")

# excel
wb.save('deliverootwitter20170306.csv')
